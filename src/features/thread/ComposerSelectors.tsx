...
function SelectorSheet({
  ...
  // Keep the sheet mounted while open only — unmounting mid-gesture via
  // parent conditionals was a crash source with nested portals.
...
export function ModelSelectorDrawer({
  ...
  return (
    <SelectorSheet title="Select model" visible={visible} onClose={onClose}>
      ...
      >
        {groups.length === 0 ? (
          <Text className="px-2 py-8 text-center text-sm text-muted">
            {safeOptions.length === 0 ? "No models available" : "No matching models"}
          </Text>
        ) : null}
        {groups.map((group) => (
...
export function ThinkingOptionsDrawer({
  ...
  readonly descriptors: readonly ProviderOptionDescriptor[];
  readonly selection: ModelSelection | null;
  readonly visible: boolean;
  readonly onClose: () => void;
  readonly onSelect: (id: string, value: ProviderOptionSelectionValue) => void;
}) {
  const safeDescriptors: readonly ProviderOptionDescriptor[] = descriptors ?? [];
                  ...
                  {descriptor.options.map((choice) => {
                    ...
                    return (
                      <Pressable
                        key={choice.id}
                        onPress={() => {
                          onSelect(descriptor.id, choice.id);
                          onClose();
                        }}
                        className={`mb-1 flex-row items-center rounded-2xl px-3 py-3 ${
                          active ? "bg-default" : ""
